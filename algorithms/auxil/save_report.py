...
def args_to_text(args_dict):
    list_of_args = [k+': '+str(v) for k,v in args_dict.items()]
    args_string = '\n'.join(list_of_args)
    return 'Arguments used for runs:\n' + args_string + '\n\n'


def record_output(prefix, oa_ae, aa_ae, kappa_ae, element_acc_ae, training_time_ae, testing_time_ae, path):
    f = open(path, 'a')
    f.write(prefix)
